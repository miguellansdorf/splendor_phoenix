defmodule SplendorWeb.UserAuthTest do
  use SplendorWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Splendor.Accounts
  alias Splendor.Accounts.Scope
  alias SplendorWeb.UserAuth

  import Splendor.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SplendorWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SplendorWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SplendorWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
